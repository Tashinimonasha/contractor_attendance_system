<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="text-medium-emphasis">Welcome back, Finance Manager</p>
    </div>
    <!-- Stat Cards -->
    <v-row>
    ...
    <v-row>
      ...
      <v-col cols="12" md="6">
        <v-card rounded="lg"><v-card-title>Contractors Cost</v-card-title><v-card-text><Bar :data="barData" :options="chartOptions" /></v-card-text></v-card>
      </v-col>
    </v-row>
  </div>
</template>
...
const barData = {
  labels: ['ABC', 'XYZ', 'Quick', 'Elite', 'Professional'],
  datasets: [{ label: 'Monthly Cost (LKR)', data: [125000, 98000, 87000, 110000, 95000], backgroundColor: '#4CAF50' }]
};
</script>
